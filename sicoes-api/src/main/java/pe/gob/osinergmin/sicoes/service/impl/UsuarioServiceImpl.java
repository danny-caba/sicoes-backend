...
//			}
	...
	public Page<Usuario> buscar(String nombreUsuario, Pageable pageable) {
		...
		for (Usuario usuario : listaUsuarios) {
			usuario.setRoles(rolDao.buscarRolUsuario2(usuario.getIdUsuario()));
		}
	...
	public ReasignacionDTO registrarUsuarioReasignacion(ReasignacionDTO dto,Contexto contexto) {
		...
		//en caso de que se este editando
		if (dto.getIdUsuarioOrigen().compareTo(dto.getIdUsuarioReasignado())!=0) {
			List<Asignacion> lista = usuarioEvaluacionDao.obtenerAsignacionesPorIdUsuario(dto.getIdUsuarioOrigen(),dto.getIdConfiguracionBandeja());
			for (Asignacion asignacion : lista) {
				asignacionDao.actualizarUsuario(asignacion.getIdAsignacion(), dto.getIdUsuarioOrigen(),null);
			}
			
			UsuarioReasignacion usReasignacionInactivo = usuarioReasignacionDao.obtenerReasignacionPorIdConf(dto.getIdConfiguracionBandeja());
			
			if (usReasignacionInactivo!=null) {
				usReasignacionInactivo.setEstadoUsuarioReasignacion(Constantes.ESTADO.INACTIVO);
				usuarioReasignacionDao.save(usReasignacionInactivo);
			}
		}
		...
		if (dto.getIdUsuarioReasignado()!=null) {
			...
			List<Asignacion> listaAsignacion = usuarioEvaluacionDao.obtenerAsignacionesPorIdUsuario(dto.getIdUsuarioOrigen(),dto.getIdConfiguracionBandeja());
			for (Asignacion asignacion : listaAsignacion) {
				asignacionDao.actualizarUsuario(asignacion.getIdAsignacion(), dto.getIdUsuarioReasignacion(),dto.getIdUsuarioOrigen());
			}
		}
