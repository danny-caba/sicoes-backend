...
public class OtroRequisitoServiceImpl implements OtroRequisitoService{
	...
	public OtroRequisito guardar(OtroRequisito otroRequisito, Contexto contexto) {
		...
		if(otroRequisito.getTipo().getCodigo().equals(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.PERFIL)) {			
			...
			if(isJuridica || isPerNatPostor||isExtrajero) {
				...
				if(otroRequisito.getSubsector()==null&&otroRequisito.getSubsector().getIdListadoDetalle()==null) {
					throw new ValidacionException(Constantes.CODIGO_MENSAJE.P_SUBSECTOR_NO_ENVIADO);
				}
				if(otroRequisitoDao.exiteRequisito(solicitudBD.getIdSolicitud(),otroRequisito.getSector().getIdListadoDetalle(),otroRequisito.getSubsector().getIdListadoDetalle(),otroRequisito.getIdOtroRequisito())>0) {
				...
				if(otroRequisito.getActividad()==null&&otroRequisito.getActividad().getIdListadoDetalle()==null) {
					throw new ValidacionException(Constantes.CODIGO_MENSAJE.P_ACTIVIDAD_NO_ENVIADO);
				}
				if(otroRequisito.getSubCategoria()==null&&otroRequisito.getSubCategoria().getIdListadoDetalle()==null) {
					throw new ValidacionException(Constantes.CODIGO_MENSAJE.P_SUBCATEGORIA_NO_ENVIADO);
				}
				if(otroRequisito.getSubCategoria()==null&&otroRequisito.getUnidad().getIdListadoDetalle()==null) {
				...
				if(otroRequisitoDao.exiteRequisito(solicitudBD.getIdSolicitud(),otroRequisito.getPerfil().getIdListadoDetalle(),otroRequisito.getIdOtroRequisito())>0) {
		...
		if(otroRequisitoBD!=null) {
			...
			if(isJuridica || isPerNatPostor||isExtrajero) {
				...
				if(otroRequisito.getSector() != null) {
					DictamenEvaluacion dictamenXSol = dictamenEvaluacionService.obtenerXSol(solicitudBD.getIdSolicitud(),otroRequisito.getSector().getIdListadoDetalle(), contexto);
			...
			otroRequisito.setTipo(listadoDetalleService.obtenerListadoDetalle(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.CODIGO,otroRequisito.getTipo().getCodigo()));
		...
		if(Constantes.LISTADO.ESTADO_SOLICITUD.OBSERVADO.equals(solicitudBD.getEstado().getCodigo())&&
				Constantes.LISTADO.RESULTADO_EVALUACION.OBSERVADO.equals(otroRequisito.getEvaluacion().getCodigo())	) {
			otroRequisitoBD.setEvaluacion(listadoDetalleService.obtenerListadoDetalle(Constantes.LISTADO.RESULTADO_EVALUACION.CODIGO,Constantes.LISTADO.RESULTADO_EVALUACION.RESPONDIDO));
		}
		...
		boolean isExtrajero = Constantes.LISTADO.TIPO_PERSONA.EXTRANJERO.equals(tipoPersona.getCodigo());
		if(isJuridica || isPerNatPostor||isExtrajero) {
			List<OtroRequisito> listEnergetico = otroRequisitoDao.listarOtroRequisitoXSector(Constantes.LISTADO.SECTOR.SECTOR_ENERGETICO, solicitudBD.getIdSolicitud()); 
			List<OtroRequisito> listMinero = otroRequisitoDao.listarOtroRequisitoXSector(Constantes.LISTADO.SECTOR.SECTOR_MINERO, solicitudBD.getIdSolicitud()); 

			if(listEnergetico.isEmpty()) {
				DictamenEvaluacion dictamenEnergetico = dictamenEvaluacionService.obtenerXCodigoSector(solicitudBD.getIdSolicitud(),Constantes.LISTADO.SECTOR.SECTOR_ENERGETICO, contexto);
				if(dictamenEnergetico != null) {
					dictamenEvaluacionService.eliminarXCodigoSector(Constantes.LISTADO.SECTOR.SECTOR_ENERGETICO,solicitudBD.getIdSolicitud(), contexto);
				}
			}
			if(listMinero.isEmpty()) {
				DictamenEvaluacion dictamenMinero = dictamenEvaluacionService.obtenerXCodigoSector(solicitudBD.getIdSolicitud(),Constantes.LISTADO.SECTOR.SECTOR_MINERO, contexto);
				if(dictamenMinero != null) {
					dictamenEvaluacionService.eliminarXCodigoSector(Constantes.LISTADO.SECTOR.SECTOR_MINERO,solicitudBD.getIdSolicitud(), contexto);
				}
			}
	...
	private void actualizarNombreArchivo(Solicitud solicitud,Contexto contexto) {
		...
		List<OtroRequisito> otrosRequisitos=listarOtroRequisito(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.OTRO_REQUISITO,solicitud.getIdSolicitud());
		int indice=1;
		logger.info("Nombres de Otro requisito");
		for(OtroRequisito otroRequisito:otrosRequisitos) {
			List<Archivo> archivosOtros=archivoService.buscar(null,null,otroRequisito.getIdOtroRequisito(),contexto);
			for(Archivo archivoAux:archivosOtros) {
				archivoAux.setCorrelativo(Long.parseLong(indice+""));
				if(Constantes.LISTADO.RESULTADO_EVALUACION.RESPONDIDO.equals(otroRequisito.getEvaluacion().getCodigo())) {
				...
				if(Constantes.LISTADO.TIPO_ARCHIVO.DNI.equals(archivoAux.getTipoArchivo().getCodigo())) {
					nombre=archivoAux.getTipoArchivo().getValor()+"_"+solicitud.getPersona().getNumeroDocumento()+"_v"+archivoAux.getVersion()+".pdf";	
				}else if (Constantes.LISTADO.TIPO_ARCHIVO.RUC.equals(archivoAux.getTipoArchivo().getCodigo())) {
					nombre=archivoAux.getTipoArchivo().getValor()+"_"+solicitud.getPersona().getCodigoRuc()+"_v"+archivoAux.getVersion()+".pdf";
					...
					if(solicitud.getUsuario().isPesonaJuridica()){
						 nombre=archivoAux.getTipoArchivo().getValor()+"_PJ_"+solicitud.getPersona().getCodigoRuc()+"_v"+archivoAux.getVersion()+".pdf";	
					} else {
						 nombre=archivoAux.getTipoArchivo().getValor()+"_PN_"+solicitud.getPersona().getNumeroDocumento()+"_v"+archivoAux.getVersion()+".pdf";
					}					
				}else if (Constantes.LISTADO.TIPO_ARCHIVO.REGISTRAL.equals(archivoAux.getTipoArchivo().getCodigo())) {
					 nombre=archivoAux.getTipoArchivo().getValor()+"_PJ_"+solicitud.getPersona().getCodigoRuc()+"_v"+archivoAux.getVersion()+".pdf";	
				}else if(Constantes.LISTADO.TIPO_ARCHIVO.ACREDITACION.equals(archivoAux.getTipoArchivo().getCodigo())) {
					 nombre=archivoAux.getTipoArchivo().getValor()+"_PJ_"+solicitud.getPersona().getCodigoRuc()+"_v"+archivoAux.getVersion()+".pdf";	
				}else if(Constantes.LISTADO.TIPO_ARCHIVO.CONSTANCIA_APROBACION.equals(archivoAux.getTipoArchivo().getCodigo())) {
					 nombre=archivoAux.getTipoArchivo().getValor()+"_v"+archivoAux.getVersion()+".pdf";	
				}else if(Constantes.LISTADO.TIPO_ARCHIVO.CONSTITUCION_EMPRESA.equals(archivoAux.getTipoArchivo().getCodigo())) {
					nombre=archivoAux.getTipoArchivo().getValor()+"_v"+archivoAux.getVersion()+".pdf";
				}else {
	...
	public void eliminar(Long id, Contexto contexto) {
		...
		if(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.PERFIL.equals(requisito.getTipo().getCodigo())) {
			Solicitud solicitud = solicitudService.obtenerSinAsignados(requisito.getSolicitud().getIdSolicitud(), contexto);

			boolean isJuridica = Constantes.LISTADO.TIPO_PERSONA.JURIDICA.equals(solicitud.getPersona().getTipoPersona().getCodigo());
			boolean isPerNatPostor = Constantes.LISTADO.TIPO_PERSONA.PN_POSTOR.equals(solicitud.getPersona().getTipoPersona().getCodigo());
			boolean isExtrajero = Constantes.LISTADO.TIPO_PERSONA.EXTRANJERO.equals(solicitud.getPersona().getTipoPersona().getCodigo());
			if(isJuridica || isPerNatPostor||isExtrajero) {
				...
				if(Constantes.LISTADO.SECTOR.SECTOR_MINERO.equals(requisito.getSector().getCodigo())) {
					...
					List<OtroRequisito> list = otroRequisitoDao.listarOtroRequisitoXSector(  requisito.getSector().getIdListadoDetalle(), solicitud.getIdSolicitud()); 
					if(list.size() == 1) {
						dictamenEvaluacionService.eliminarXSector(requisito, contexto);
					}
				}
	...
	public OtroRequisito evalular(OtroRequisito otroRequisito, Contexto contexto) {
		...
		if(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.OTRO_REQUISITO.equals(otroRequisito.getTipo().getCodigo())) {
			...
			if(otroRequisito.getTipoRequisito()==null&&otroRequisito.getTipoRequisito().getIdListadoDetalle()==null) {
	...
	public OtroRequisito finalizar(OtroRequisito otroRequisito, Contexto contexto) {
		...
		OtroRequisito otroRequisitoBD=otroRequisitoDao.obtener(otroRequisito.getIdOtroRequisito());
		if (otroRequisitoBD.getEvaluacion()!=null&&Constantes.LISTADO.RESULTADO_EVALUACION.POR_EVALUAR.equals(otroRequisito.getEvaluacion().getCodigo())) {
			throw new ValidacionException(Constantes.CODIGO_MENSAJE.EVALUE_TODO_REQUISITOS);
		}
		
		ListadoDetalle finalizado=listadoDetalleService.obtenerListadoDetalle(Constantes.LISTADO.SI_NO.CODIGO, Constantes.LISTADO.SI_NO.SI);
	...
	public List<OtroRequisito> listarOtroRequisitoXSolicitudPJ(Long idSolicitud,Long idUsuario, Contexto contexto) {
		return otroRequisitoDao.listarOtroRequisitoXSolicitudPJ(idSolicitud,idUsuario);
	}
	...
	public OtroRequisito asignarEvaluadorPerfil(Long idOtroRequisito, List<Asignacion> asignaciones, Contexto contexto) {
		...
		List<Asignacion> listaAsiganaciones = asignacionDao.obtenerAsignaciones(otroRequisito.getSolicitud().getIdSolicitud());
		...
		PerfilDivision perfilDivision = perfilDivisionDao.obtenerPerfilDivisionPorIdPerfil(otroRequisito.getPerfil().getIdListadoDetalle());
		List<PerfilAprobador> listaPerfilesAprobadores = perfilAprobadorDao.obtenerPerfilAprobadorPorIdPerfil(otroRequisito.getPerfil().getIdListadoDetalle());
		...
		if (listaPerfilesAprobadores != null && !listaPerfilesAprobadores.isEmpty()) {
			...
			for (Asignacion asignacion : listaAsiganaciones) {
				/*if (asignacion.getOtroRequisito() != null) {
					PerfilDivision perfilDivisionAprobadores = perfilDivisionDao.obtenerPerfilDivisionPorIdPerfil(asignacion.getOtroRequisito().getPerfil().getIdListadoDetalle());
					if (perfilDivision.getDivision().getIdDivision().equals(perfilDivisionAprobadores.getDivision().getIdDivision())) {
						registrarAsignacion = false;
					}
				}*/
				
				if(asignacion.getGrupo() != null) {
					if(asignacion.getGrupo().getCodigo().equals(Constantes.LISTADO.GRUPOS.G1) && 
					   !asignacion.getUsuario().getIdUsuario().equals(perfilAprobadores.getAprobadorG1().getIdUsuario())) {
                    ...
                    if(asignacion.getGrupo().getCodigo().equals(Constantes.LISTADO.GRUPOS.G2) && 
					   asignacion.getUsuario().getIdUsuario().equals(perfilAprobadores.getAprobadorG1().getIdUsuario())) {
			...
			if (registrarAsignacion) {
				...
				List<ListadoDetalle> tipos = listadoDetalleService.listarListadoDetallePorCoodigo(Constantes.LISTADO.TIPO_EVALUADOR.APROBADOR_TECNICO, contexto);
				...
				if (!perfilDivision.getDivision().getIdDivision().equals(Constantes.DIVISION.IDENTIFICADOR.ID_GSM)) {
					...
					List<ListadoDetalle> grupos1 = listadoDetalleService.listarListadoDetallePorCoodigo(Constantes.LISTADO.GRUPOS.G1, contexto);
					...
					List<ListadoDetalle> grupos2 = listadoDetalleService.listarListadoDetallePorCoodigo(Constantes.LISTADO.GRUPOS.G2, contexto);
			...
			logger.error("No se econtro relacion de aprobadores a pefil : " + otroRequisito.getPerfil().getIdListadoDetalle());
		}
	...
	public void copiarOtrosRequisitosUltimaSolicitud(String solicitudUuidUltima, String solicitudUuid, Contexto contexto) {
		...
		List<OtroRequisito> listaOtrosRequisitosUltimaSolicitud = otroRequisitoDao.listarOtroRequisito(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.OTRO_REQUISITO, idSolicitudUltima);
		List<OtroRequisito> listaOtrosRequisitosSolicitud = otroRequisitoDao.listarOtroRequisito(Constantes.LISTADO.TIPO_DOCUMENTO_ACREDITA.OTRO_REQUISITO, idSolicitud);
		...
		if (listaOtrosRequisitosUltimaSolicitud != null) {
			...
			for (OtroRequisito otroRequisito : listaOtrosRequisitosUltimaSolicitud) {
				...
				ListadoDetalle evaluacion = listadoDetalleDao.listarListadoDetallePorCoodigo(Constantes.LISTADO.RESULTADO_EVALUACION.POR_EVALUAR).get(0);
				...
				List<Archivo> listaArchivos = archivoService.buscar(null,null,otroRequisito.getIdOtroRequisito(), contexto);
	...
	public OtroRequisito actualizarEstadoSolicitudRevertirEvaluacion(OtroRequisito otroRequisito, Contexto contexto) {
		...
		if (!solicitud.getFlagActivo().equals(1L) || !solicitud.getEstado().getCodigo().equals(Constantes.LISTADO.ESTADO_SOLICITUD.EN_PROCESO)) {
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public OtroRequisito modificarEvaluadorPerfil(Long idOtroRequisito, List<Asignacion> asignaciones,Contexto contexto) {
		OtroRequisito otroRequisito = otroRequisitoDao.obtener(idOtroRequisito);
	if (otroRequisito.getFinalizado()==null) {
			...
			for (Asignacion asignacion : listaAsiganaciones) {
				if (asignacion.getTipo().getCodigo().equals(Constantes.LISTADO.TIPO_EVALUADOR.TECNICO) && asignacion.getOtroRequisito().getIdOtroRequisito().equals(otroRequisitoRpt.getIdOtroRequisito())) {
