...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers(HttpMethod.GET,"/api/supervisora-perfiles").permitAll()		
		.antMatchers(HttpMethod.GET,"/api/supervisora-perfiles/**").permitAll()	
		...
		.antMatchers("/api/consulta-reniec").permitAll()		
		.antMatchers(HttpMethod.GET, "/api/departamentos").permitAll()
		.antMatchers(HttpMethod.GET, "/api/provincias/**").permitAll()
		.antMatchers(HttpMethod.GET, "/api/distritos/**").permitAll()
		.antMatchers(HttpMethod.GET, "/api/descargar-observaciones-url/**").permitAll()
		...
		.antMatchers(HttpMethod.GET, "/api/listado/**//*detalles").permitAll()
		.antMatchers(HttpMethod.POST, "/api/solicitudes-registro-usuario").permitAll()
		...
		.antMatchers(HttpMethod.POST, "/api/usuarios/token").authenticated()
		.anyRequest().authenticated()
		.and().cors().configurationSource(corsConfigurationSource());
	}
	
	 /*
	 public ClienteSissegConfig clienteSissegConfig() {
		 ClienteSissegConfig clienteSissegConfig=new ClienteSissegConfig();		 
		 clienteSissegConfig.put(ClienteSissegConfigParams.BASE_URL, env.getProperty("sisseg.base-url"));
		 clienteSissegConfig.put(ClienteSissegConfigParams.ENCRYPTION_KEY,env.getProperty("sisseg.application-id") );
		 clienteSissegConfig.put(ClienteSissegConfigParams.APPLICATION_ID,env.getProperty("sisseg.encryption-key") );
		 return clienteSissegConfig;
	 }
	 
	 
	 public SissegAuthenticationProvider sissegAuthenticationProvider() {
		 return new SissegAuthenticationProvider(clienteSissegConfig());
	 }
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception{
		SissegAuthenticationConfigurer<HttpSecurity> configurer=new SissegAuthenticationConfigurer<HttpSecurity>(sissegAuthenticationProvider());
	...
	public CorsConfigurationSource corsConfigurationSource() {
		...
		config.setAllowedOrigins(Arrays.asList(CorsConfiguration.ALL, "http://localhost:4200","http://078d-190-234-127-88.ngrok.io"));
//		config.setAllowedOrigins(Arrays.asList(CorsConfiguration.ALL));
		...
		List<String> headers = Arrays.asList("Access-Control-Expose-Headers", "Access-Control-Allow-Headers",
				...
				"Origin", "Cache-Control", "Content-Type", "Authorization", "Error-Code", "Error-Message","Application","IdEmpresa");
